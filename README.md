...
- `backtester.py`: Historical simulation engine that aligns past regime states with forward sector returns to evaluate strategy performance against an equal-weight benchmark.
...
*(Note: Requires `matplotlib` installed in your environment to render the equity curve plot.)*
...
**`backtester.py` — what it does and method used**

* Purpose: historical simulation of the AQUA allocation strategy to evaluate performance metrics and drawdowns.
* Process:
* Aggregates the historical HMM regime labels generated by `regime_detection.py`.
* Implements a temporal shift (lag) to align end-of-month regime states with the *following* month's actual sector returns, ensuring realistic execution timing.
* Computes monthly portfolio returns as a weighted linear combination of sector forward returns, based on the weights prescribed by `weight_allocation.py`.
* Calculates cumulative wealth and standard performance metrics (CAGR, Annual Volatility, Sharpe Ratio, Maximum Drawdown).
* Compares the AQUA strategy against a naive $1/N$ equal-weight benchmark to evaluate the alpha generated by the dynamic regime-switching rules.


* Caveat (Lookahead Bias): In the current iteration, the underlying HMM fits on the entire dataset simultaneously. For strict out-of-sample testing, the regime detection module would need to be refactored into an expanding window framework.
